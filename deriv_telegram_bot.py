...
import asyncio import websockets import json import pandas as pd from ta.momentum import RSIIndicator from ta.trend import EMAIndicator, MACD from ta.volatility import BollingerBands import requests from datetime import datetime, timedelta, timezone from dotenv import load_dotenv import os from pathlib import Path import time import random import logging import traceback from collections import deque import html from flask import Flask import threading
...
logger = logging.getLogger("indicador") logger.setLevel(logging.INFO) handler = logging.StreamHandler() formatter = logging.Formatter("%(asctime)s | %(levelname)s | %(message)s","%Y-%m-%dT%H:%M:%S") handler.setFormatter(formatter) logger.addHandler(handler) def log(msg:str,level:str="info"): if level=="info": logger.info(msg) elif level=="warning": logger.warning(msg) elif level=="error": logger.error(msg) else: logger.debug(msg)

---------------- Telegram ----------------

def send_telegram(message:str,symbol:str=None,bypass_throttle:bool=False): now_ts=time.time() if symbol and not bypass_throttle: last=last_notify_time.get(symbol,0) if now_ts-last<3: log(f"[TG] throttle skip for {symbol}", "warning") return last_notify_time[symbol]=now_ts if not TELEGRAM_TOKEN or not CHAT_ID: log("⚠️ Telegram não configurado (TELEGRAM_TOKEN/CHAT_ID faltando).", "warning") return try: url=f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage" payload={"chat_id":CHAT_ID,"text":message,"parse_mode":"HTML","disable_web_page_preview":True} r = requests.post(url,data=payload,timeout=10) if r.status_code != 200: log(f"❌ Telegram retornou {r.status_code}: {r.text}", "error") except Exception as e: log(f"[TG] Erro ao enviar: {e}", "error")

---------------- Utilitários ----------------

def human_pair(symbol:str)->str: return symbol.replace("frx","")

def calcular_indicadores(df:pd.DataFrame)->pd.DataFrame: if df is None or df.empty: return pd.DataFrame() df=df.sort_values("epoch").reset_index(drop=True) for c in ["open","high","low","close","volume"]: df[c]=pd.to_numeric(df.get(c,0.0),errors="coerce").fillna(0.0) # cálculos de EMA/RSI/MACD/Bollinger df[f"ema{EMA_FAST}"]=EMAIndicator(df["close"],EMA_FAST).ema_indicator() df[f"ema{EMA_MID}"]=EMAIndicator(df["close"],EMA_MID).ema_indicator() df[f"ema{EMA_SLOW}"]=EMAIndicator(df["close"],EMA_SLOW).ema_indicator() df["rsi"]=RSIIndicator(df["close"],14).rsi().fillna(50.0) try: macd=MACD(df["close"],26,12,9) df["macd_diff"]=macd.macd_diff().fillna(0.0) except Exception: df["macd_diff"]=0.0 bb=BollingerBands(df["close"],window=20,window_dev=2) df["bb_upper"]=bb.bollinger_hband().fillna(df["close"]) df["bb_lower"]=bb.bollinger_lband().fillna(df["close"]) df["bb_mavg"]=bb.bollinger_mavg().fillna(df["close"]) df["bb_width"]=df["bb_upper"]-df["bb_lower"] df["rel_sep"]=(df[f"ema{EMA_MID}"]-df[f"ema{EMA_SLOW}"]).abs()/df["close"].replace(0,1e-12) return df

---------------- ML ----------------

def _build_ml_dataset(df:pd.DataFrame): df2=df.copy().reset_index(drop=True) features=["open","high","low","close","volume", f"ema{EMA_FAST}",f"ema{EMA_MID}",f"ema{EMA_SLOW}", "rsi","macd_diff","bb_upper","bb_lower","bb_mavg","bb_width","rel_sep"] for c in features: df2[c]=df2.get(c,0.0) y=(df2["close"].shift(-1)>df2["close"]).astype(int) X=df2.iloc[:-1].copy() y=y.iloc[:-1].copy() if len(X)>ML_MAX_SAMPLES: X=X.tail(ML_MAX_SAMPLES).reset_index(drop=True) y=y.tail(ML_MAX_SAMPLES).reset_index(drop=True) return X,y
...
def check_and_activate_fallback(): prune_sent_timestamps() global fallback_active_until if len(sent_timestamps)<MIN_SIGNALS_PER_HOUR: fallback_active_until=time.time()+FALLBACK_DURATION_SECONDS log("⚠️ Fallback ativado (poucos sinais recentes).", "warning")

def is_fallback_active(): return time.time()<fallback_active_until
...
def gerar_sinal(df:pd.DataFrame,symbol:str): try: if len(df)<EMA_SLOW+5: log(f"[{symbol}] Dados insuficientes ({len(df)} candles).", "info") return None now=df.iloc[-1] candle_id=int(now["epoch"])-(int(now["epoch"])%GRANULARITY_SECONDS) if last_signal_candle.get(symbol)==candle_id: return None ema_fast=float(now[f"ema{EMA_FAST}"]) ema_mid=float(now[f"ema{EMA_MID}"]) ema_slow=float(now[f"ema{EMA_SLOW}"]) triple_up=ema_fast>ema_mid>ema_slow triple_down=ema_fast<ema_mid<ema_slow close=float(now["close"]) bb_upper=float(now["bb_upper"]) bb_lower=float(now["bb_lower"]) width=bb_upper-bb_lower if bb_upper-bb_lower!=0 else 1.0 perto_lower=close<=bb_lower+widthBB_PROXIMITY_PCT perto_upper=close>=bb_upper-widthBB_PROXIMITY_PCT bullish=now["close"]>now["open"] bearish=now["close"]<now["open"] rsi_now=float(now["rsi"]) if not pd.isna(now["rsi"]) else 50.0 macd_diff=now.get("macd_diff") macd_buy_ok=True if macd_diff is None or pd.isna(macd_diff) else macd_diff>-MACD_TOLERANCE macd_sell_ok=True if macd_diff is None or pd.isna(macd_diff) else macd_diff<MACD_TOLERANCE buy_ok=triple_up and (bullish or perto_lower) and rsi_now<=RSI_BUY_MAX and macd_buy_ok sell_ok=triple_down and (bearish or perto_upper) and rsi_now>=RSI_SELL_MIN and macd_sell_ok if is_fallback_active(): if not buy_ok and ema_mid>ema_slow and bullish: buy_ok=True if not sell_ok and ema_mid<ema_slow and bearish: sell_ok=True

log(f"[{symbol}] EMA{EMA_FAST}={ema_fast:.6f} EMA{EMA_MID}={ema_mid:.6f} EMA{EMA_SLOW}={ema_slow:.6f} RSI={rsi_now:.2f} MACD_diff={macd_diff}", "info")
...
async def monitor_symbol(symbol:str): """ Monitor individual symbol com reconexão/backoff e logs. Correção chave: envia 'subscribe':1 junto ao pedido de ticks_history para receber atualizações ao vivo. """ columns = ["epoch","open","high","low","close","volume"] df = pd.DataFrame(columns=columns)
...
while True:
    try:
        ...
        async with websockets.connect(WS_URL, ping_interval=40, ping_timeout=20, max_size=None) as ws:
            ...
            # solicita histórico E pede subscribe para updates ao vivo
            # >>> CORREÇÃO IMPORTANTE: adicionamos "subscribe": 1
            subscribe_msg={"ticks_history":symbol,
                           "adjust_start_time":1,
            ...
            log(f"{symbol} | Histórico solicitado ({INITIAL_HISTORY_COUNT} candles) + subscribe para updates ao vivo.", "info")
            ...
            while True:
                ...
                # tratamento mensagens: history (inicial), candle/ohlc/candles atualizações
                candle = None

                # histórico inicial retornado em 'history' ou 'history' -> 'candles'
                    ...
                    try:
                        ...
                        if isinstance(candles, list) and len(candles) > 0:
                            ...
                            if not hist.empty:
                                ...
                                # treinar ML ao carregar histórico se possível
                                try:
                                    if ML_ENABLED:
                ...
                # atualização de candle fechada (algumas respostas podem usar 'candle', 'ohlc' ou 'candles')
                if "candle" in msg and isinstance(msg.get("candle"), dict):
                    ...
                    # mensagem contendo lista com candles recentes - pega última
                    candle = msg["candles"][-1]
                elif "tick" in msg and isinstance(msg.get("tick"), dict):
                    # ticks brutos (caso receba), ignoramos para manter lógica de candles
                    continue

                if not candle:
                    # heartbeat ou outro tipo
                    continue

                # parse candle (esperamos candles alinhadas à granularidade)
                try:
                    epoch = int(candle.get("epoch"))
                    # aceitar candles apenas fechadas alinhadas à granularidade
                    if epoch % GRANULARITY_SECONDS != 0:
                        # pode ser candle em tempo real com epoch de início; tolerância: aceitar se for múltiplo de 1 segundo e próximo
                        # porém para evitar duplicatas, apenas pular
                        continue
...
app=Flask(name) @app.get("/") def home(): return "BOT ONLINE",200
