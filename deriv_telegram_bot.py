...
def _build_ml_dataset(df: pd.DataFrame):
    ...
    df2["future_up"] = (df2["close"].shift(-1) > df2["close"]).astype(int)
    y = df2["future_up"].iloc[:-1]
    feature_cols = [c for c in df2.columns if c not in ["future_up", "epoch"]]
    X = df2[feature_cols].iloc[:-1]
...
def train_ml_for_symbol(df: pd.DataFrame, symbol: str) -> bool:
    ...
    try:
        ...
        if len(X) < ML_MIN_TRAINED_SAMPLES:
            ...
            return False
        from sklearn.ensemble import RandomForestClassifier
        model = RandomForestClassifier(n_estimators=ML_N_ESTIMATORS, max_depth=ML_MAX_DEPTH)
        model.fit(X, y)
        ml_models[symbol] = (model, X.columns.tolist())
        ml_model_ready[symbol] = True
        log(f"[ML {symbol}] Modelo treinado com {len(X)} samples.")
        return True
    except Exception as e:
        ml_model_ready[symbol] = False
        log(f"[ML {symbol}] Falha ao treinar: {e}", "error")
        return False
...
def gerar_sinal(df: pd.DataFrame, symbol: str):
    try:
        ...
        if last_signal_candle[symbol] == candle_id:
            return None
        ...
        if ml_prob is not None and ml_prob < ML_CONF_THRESHOLD:
            log(f"{symbol} | Sinal bloqueado pelo ML: {ml_prob*100:.1f}%")
            return None
        ...
        def delayed_send(send_ts, symbol, tipo, next_epoch, ml_prob):
            ...
            log(f"{symbol} | Sinal enviado: {tipo} (ML {ml_prob*100:.1f}%)")
...
async def monitor_symbol(symbol: str):
    df = pd.DataFrame(columns=["epoch","open","high","low","close","volume"])
    connect_attempt = 0
...
# ---------------- MAIN ----------------
async def main():
    send_telegram("🤖 BOT INICIADO!", bypass_throttle=True)
