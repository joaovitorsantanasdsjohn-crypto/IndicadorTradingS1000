            ...
            async with websockets.connect(WS_URL, ping_interval=None) as ws:
                ...
                while True:
                    try:
                        ...
                        if time.time() - ultimo_candle > 300:
                            print(f"⚠️ [{symbol}] Nenhum candle há 5min — reconectando...")
                            raise Exception("Reconexão forçada por inatividade")
                        else:
                            print(f"⏱ [{symbol}] Aguardando novo candle...")
                            continue
        except Exception as e:
...
async def main():
    ...
    print("▶ Iniciando monitoramento paralelo por par (modo estável com reconexão automática)...")
